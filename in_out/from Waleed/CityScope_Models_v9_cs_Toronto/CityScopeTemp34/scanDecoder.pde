...
public class TagDecoder {
  ...
  private int U, V, W, X;
  private int[][] rotation, id, use, quadcode;
      ...
      for (int v=0; v<V; v++) { 
        ...
        for (int i=0; i<W; i++) {
          for (int j=0; j<X; j++) {
            ...
            if (W == 2) { 
              ...
              if (quadCode[u][v][i][j] == 2 || quadCode[u][v][i][j] == 3 || quadCode[u][v][i][j] == 1) {
              ...
              //Generates code for 1-bit color. does not allow rotation
              else if (W == 1) { 
                id[u][v] += quadCode[u][v][i][j];
              }
            }
          }
        }
        ...
        for (int i=0; i<buildingDef.length; i++) {
          if (buildingDef[i][0] == id[u][v]) {
            ...
            quadcode[u][v] = (int)buildingDef[i][0];
            break;
          }
          if (!matched) {
            id[u][v] =  id[u][v];
